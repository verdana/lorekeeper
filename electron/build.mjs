...
// 从 .env.local 里取提示词语言（PROMPT_LANG / VITE_PROMPT_LANG 互为别名），
// 构建期烧进主进程 bundle。运行时主进程不带 .env.local，直接读 process.env
// 永远拿不到值，会退回英文。
function readPromptLang() {
  const fromEnv = process.env.PROMPT_LANG || process.env.VITE_PROMPT_LANG
  ...
  for (const raw of readFileSync('.env.local', 'utf8').split(/\r?\n/)) {
    ...
    if (key === 'PROMPT_LANG' || key === 'VITE_PROMPT_LANG') {
      return line
        .slice(eq + 1)
        .trim()
        .replace(/^["']|["']$/g, '')
